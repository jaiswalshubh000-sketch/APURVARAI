...
async function run() {
  ...
  for (let i = 0; i < rawUrls.length; i++) {
    ...
    try {
      ...
      console.log(`${i+1}/${rawUrls.length} ID: ${id} -> Category: ${category}`);
      
      if (results[category]) {
        results[category].push(`"https://drive.google.com/file/d/${id}/view?usp=sharing"`);
      } else {
        console.log(`Unknown category ${category} for ${id}`);
      }
    } catch (e) {
